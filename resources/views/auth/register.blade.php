    ...
    <div class="w-full max-w-md bg-white rounded-[32px] border border-gray-100 shadow-2xl p-8 space-y-6">
        ...
        <div class="text-center space-y-2">
            ...
            <div class="pt-2">
                ...
                <p class="text-xs font-bold text-gray-400">Daftarkan bisnis UMKM Anda untuk mulai mengelola workspace</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-4">
            ...
            <div class="space-y-1.5">
                ...
                @if($errors->has('name'))
                    <p class="text-red-500 text-2xl font-bold mt-1 pl-1" style="font-size: 11px;">{{ $errors->first('name') }}</p>
                @endif
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <input id="email" type="email" name="email" :value="old('email')" required autocomplete="username" placeholder="nama@email.com"
                    class="w-full bg-[#F8FAFC] border border-gray-100 rounded-xl px-4 py-3 text-sm font-semibold text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#9333EA]/20 focus:border-[#9333EA] transition">
                @if($errors->has('email'))
                    <p class="text-red-500 text-2xl font-bold mt-1 pl-1" style="font-size: 11px;">{{ $errors->first('email') }}</p>
                @endif
            </div>
            ...
            <div class="space-y-1.5">
                ...
                @if($errors->has('password'))
                    <p class="text-red-500 text-2xl font-bold mt-1 pl-1" style="font-size: 11px;">{{ $errors->first('password') }}</p>
                @endif
            </div>
            ...
            <div class="space-y-1.5">
                ...
                @if($errors->has('password_confirmation'))
                    <p class="text-red-500 text-2xl font-bold mt-1 pl-1" style="font-size: 11px;">{{ $errors->first('password_confirmation') }}</p>
                @endif
            </div>
        ...
        <div class="text-center pt-2 border-t border-gray-50">
            <p class="text-xs font-bold text-gray-400">
                Sudah punya akun? 
                <a href="{{ route('login') }}" class="text-[#9333EA] hover:text-[#7e22ce] underline transition ml-1">
                    Masuk di sini
                </a>
            </p>
        </div>
