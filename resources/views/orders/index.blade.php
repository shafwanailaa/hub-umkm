     ...
     x-data="{ 
        ...
        <header class="border-b border-gray-100 px-8 py-6 flex justify-between items-center">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="text-gray-400 hover:text-red-500 font-bold transition">Logout</button>
            </form>
        </header>

        <main class="p-8 flex-grow space-y-8">
            <div class="flex gap-3 overflow-x-auto pb-4 no-scrollbar">
                <template x-for="status in ['Semua', 'Pending', 'Diproses', 'Dikirim']" :key="status">
                    <button @click="currentStatus = status" 
                            :class="currentStatus === status ? 'bg-[#9333EA] text-white shadow-lg' : 'bg-white text-gray-400 border border-gray-100'" 
