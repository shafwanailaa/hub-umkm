        ...
        <main class="p-6 space-y-6 flex-grow mb-24">
            ...
            @if(session('success'))
                <div x-data="{ show: true }" 
                     ...
                     class="bg-emerald-50 border border-emerald-100 text-emerald-600 text-xs font-bold p-4 rounded-2xl text-center shadow-sm animate-pulse">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="bg-white p-5 rounded-[24px] border-2 border-blue-400 shadow-lg shadow-blue-50 space-y-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h4 class="font-[900] text-gray-800 text-lg leading-tight">{{ $pesanan->nama_pembeli ?? $pesanan->nama ?? 'Siska Amalia' }}</h4>
                    </div>
                </div>
                <div class="flex items-center gap-4">
                    <div class="bg-green-400 p-3 rounded-2xl text-white shadow-md shadow-green-50">
                        <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 00-2 2z" /></svg>
                    </div>
                    <div>
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Tanggal Pesanan</p>
                        <h4 class="font-[900] text-gray-800 text-lg leading-tight">
                            {{ isset($pesanan->created_at) ? \Carbon\Carbon::parse($pesanan->created_at)->translatedFormat('d/m/Y, H.i') : '27/4/2026, 09.31' }}
                        </h4>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm space-y-4">
                ...
                <div class="bg-[#FDF2F8] p-4 rounded-2xl flex justify-between items-center">
                    <div>
                        <h5 class="font-bold text-gray-800">{{ $pesanan->nama_produk ?? 'Pensilcase Rajut' }}</h5>
                        <p class="text-[10px] font-bold text-gray-400">Qty: {{ $pesanan->qty ?? $pesanan->jumlah ?? 1 }}</p>
                    </div>
                    <span class="font-[900] text-gray-800">Rp {{ number_format($pesanan->total_harga ?? $pesanan->harga ?? 50000, 0, ',', '.') }}</span>
                </div>
                ...
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-[900] text-[#E879F9] text-xl">Rp {{ number_format($pesanan->total_harga ?? $pesanan->harga ?? 50000, 0, ',', '.') }}</span>
                </div>
            </div>
           ...
           <div class="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <form action="{{ route('orders.update-status', $primaryKeyPesanan) }}" method="POST">
                        ...
                        <button type="submit" 
                                :class="status === 'dikirim' ? 'bg-gradient-to-r from-[#9333EA] to-[#E879F9] text-white shadow-purple-200' : 'bg-gray-100 text-gray-400'" 
                                class="w-full py-4 rounded-2xl font-black text-sm shadow-md transition cursor-pointer">
                            Dikirim
                        </button>
                    </form>
                    ...
                    <form action="{{ route('orders.update-status', $primaryKeyPesanan) }}" method="POST">
                        ...
                        <button type="submit" 
                                :class="status === 'selesai' ? 'bg-gradient-to-r from-[#9333EA] to-[#E879F9] text-white shadow-purple-200' : 'bg-gray-100 text-gray-400'" 
                                class="w-full py-4 rounded-2xl font-black text-sm shadow-md transition cursor-pointer">
                            Selesai
                        </button>
                    </form>
